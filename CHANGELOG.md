...
- **The `agentsync` skill is engine-owned now, so upgrades reach every project.** It documents AgentSync itself, but it was copied into `.ai/src/skills/` at `init` and only updated if someone remembered to run `refresh` — so a team on an old copy had agents reasoning from stale behaviour. It now ships in the install dir under `lib/templates/base-src/skills/` and is resolved at sync time, layered after any `shared:` parent, so precedence reads project → shared parent → engine. Upgrade the engine and the next `sync` in any project emits the current version, with no prompt and no merge. A project copy still wins, and `base_skills: false` drops the layer. The layer is generic: any src-shaped content under `base-src/` composes the same way.
- **`format:` in `agent_sync.yaml` — a project format revision, with a reminder that only fires when a migration applies.** It is a small counter bumped when a project genuinely needs a step, unlike `agentsync_version`, which moves on every patch and would nag. `init` writes the current revision, `migrate --apply` records it, and nothing else touches it, so no command can silence a migration that still applies. A project behind the engine is flagged on the next interactive command and by `doctor`; the notice disappears permanently once recorded, so it needs no rate limiting.
- **`agentsync migrate` retires an engine-owned skill copy.** For projects scaffolded before the change: it reports the shadowing copy, and `--apply` removes it when every file still matches the hash recorded in `.ai/.template-manifest` (untouched, so nothing is lost) while leaving an edited copy in place as the deliberate override it is. Either way the format revision is recorded, so the reminder stops.
